...
// const xlsxjson = require('xlsx-to-json');
//get current date
const fromDate = Cypress.moment().format("DD-MM-YYYY")

const autoGenerate = Math.floor((Math.random() * 1000) + 1);

const username_VT = autoGenerate + createUser.usernameVT;
const fullname_VT = autoGenerate + createUser.fullnameVT;
const email_VT = autoGenerate + createUser.emailVT;
const password_VT = autoGenerate + createUser.passwordVT;

const username_GD = autoGenerate + createUser.usernameGD;
const fullname_GD = autoGenerate + createUser.fullnameGD;
const email_GD = autoGenerate + createUser.emailGD;
const password_GD = autoGenerate + createUser.passwordGD;

const username_TP = autoGenerate + createUser.usernameTP;
const fullname_TP = autoGenerate + createUser.fullnameTP;
const email_TP = autoGenerate + createUser.emailTP;
const password_TP = autoGenerate + createUser.passwordTP;

const username_NV = autoGenerate + createUser.usernameNV;
const fullname_NV = autoGenerate + createUser.fullnameNV;
const email_NV = autoGenerate + createUser.emailNV;
const password_NV = autoGenerate + createUser.passwordNV;
//=========================USING EXCELJS=========================
...
const sheetNames = ["Văn Thư Account", "Giám Đốc Account", "Trưởng Phòng Account", "Nhân Viên Account"];
...
And(/^I verify write data to excel file successfully$/, function () {
    sheetNames.forEach(sheetName => {
        ...
        switch (worksheet) {
            case workbook.getWorksheet('Văn Thư Account'):
                worksheet.columns = [
                    { header: "VT_username", key: 'VT_username' },
                    { header: "VT_fullname", key: 'VT_fullname' },
                    { header: "VT_email", key: 'VT_email' },
                    { header: "VT_password", key: 'VT_password' }
                ];

                worksheet.addRow(
                    { VT_username: username_VT, VT_fullname: fullname_VT, VT_email: email_VT, VT_password: password_VT });
                break;

            case workbook.getWorksheet('Giám Đốc Account'):
                worksheet.columns = [
                    { header: "GD_username", key: 'GD_username' },
                    { header: "GD_fullname", key: 'GD_fullname' },
                    { header: "GD_email", key: 'GD_email' },
                    { header: "GD_password", key: 'GD_password' }
                ];

                worksheet.addRow(
                    { GD_username: username_GD, GD_fullname: fullname_GD, GD_email: email_GD, GD_password: password_GD });
                break;

            case workbook.getWorksheet('Trưởng Phòng Account'):
                worksheet.columns = [
                    { header: "TP_username", key: 'TP_username' },
                    { header: "TP_fullname", key: 'TP_fullname' },
                    { header: "TP_email", key: 'TP_email' },
                    { header: "TP_password", key: 'TP_password' }
                ];

                worksheet.addRow(
                    { TP_username: username_TP, TP_fullname: fullname_TP, TP_email: email_TP, TP_password: password_TP });
                break;

            case workbook.getWorksheet('Nhân Viên Account'):
                worksheet.columns = [
                    { header: "NV_username", key: 'NV_username' },
                    { header: "NV_fullname", key: 'NV_fullname' },
                    { header: "NV_email", key: 'NV_email' },
                    { header: "NV_password", key: 'NV_password' }
                ];

                worksheet.addRow(
                    { NV_username: username_NV, NV_fullname: fullname_NV, NV_email: email_NV, NV_password: password_NV });
                break;
    ...
    workbook.xlsx.writeBuffer()
        .then(buffer => FileSaver.saveAs(new Blob([buffer]), 'Data For Create Account [' + fromDate + ']' + '.xlsx'))
        .catch(err => console.log('Error writing excel export', err))

});
And(/^I verify save file with format json file successfully$/, function () {
    const jsonArr = [{ AD_username: createUser.usernameAD, AD_password: createUser.passwordAD },
    { VT_username: username_VT, VT_fullname: fullname_VT, VT_email: email_VT, VT_password: password_VT },
    { GD_username: username_GD, GD_fullname: fullname_GD, GD_email: email_GD, GD_password: password_GD },
    { TP_username: username_TP, TP_fullname: fullname_TP, TP_email: email_TP, TP_password: password_TP },
    { NV_username: username_NV, NV_fullname: fullname_NV, NV_email: email_NV, NV_password: password_NV }];
    cy.writeFile('cypress/fixtures/testdata.json', jsonArr);
});